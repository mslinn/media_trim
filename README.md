...
* `-H` Disable hardware acceleration and use software decoding. If hardware
      acceleration cannot be initialized, the command will automatically retry using
      software decoding. Software decoding may be slower but will produce the same
      output. If a required software decoder is missing the program will halt and
...
Bug reports and pull requests are welcome at https://github.com/mslinn/media_trim.
